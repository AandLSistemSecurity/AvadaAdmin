...
app.post('/logout', (req, res) => {
    res.clearCookie('access_token', { maxAge: 3600000, httpOnly: true, domain: 'localhost', path: '/dashboard', sameSite: true })
    res.status(200).json({
        path: '/login/'
    })
})
        ...
        bcrypt.compare(req.body.password, user.password, (err, result) => {
            if (err) {
                ...
                if (result) {
                    ...
                    res.cookie('access_token', accessToken, { maxAge: 3600000, httpOnly: true, domain: 'localhost', path: '/dashboard', sameSite: true })
                    res.status(200).json({
                        path: '/dashboard/'
                    })
                } else {
